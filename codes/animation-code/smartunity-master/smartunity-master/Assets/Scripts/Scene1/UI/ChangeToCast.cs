...
public class ChangeToCast : MonoBehaviour
{
    ...
    public void OnClick()
    {
        ...
        {
            lightGroup[i].intensity = SaveGameData.Instance.CastLightIntensity;
        }
        SaveGameData.Instance.CurrentFirstLightIntensity = SaveGameData.Instance.CastLightIntensity;
        lightGroup = GameObject.Find("LightGroup2").GetComponentsInChildren<Light>();
        for (int i = 0; i < lightGroup.Length; ++i)
        {
            lightGroup[i].intensity = SaveGameData.Instance.CastLightIntensity;
        }
        SaveGameData.Instance.CurrentSecondLightIntensity = SaveGameData.Instance.CastLightIntensity;
        lightGroup = GameObject.Find("LightGroup3").GetComponentsInChildren<Light>();
        for (int i = 0; i < lightGroup.Length; ++i)
        {
            lightGroup[i].intensity = SaveGameData.Instance.CastLightIntensity;
        }
        SaveGameData.Instance.CurrentThirdLightIntensity = SaveGameData.Instance.CastLightIntensity;
        for (int i = 0; i < 4; ++i)
        {
            ani = GameObject.Find("Curtain" + i).GetComponent<Animation>();
            if (SaveGameData.Instance.CastCurtainState == "关闭" && SaveGameData.Instance.Curtain[i])
            {
            ...
            else if(SaveGameData.Instance.CastCurtainState == "打开" && !SaveGameData.Instance.Curtain[i])
            {
                ani["Curtain" + i].speed = 1.0f;
                ani.Play("Curtain" + i);
